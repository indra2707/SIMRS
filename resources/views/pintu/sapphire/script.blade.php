<script type="text/javascript">
    // Variable Name
    var $tableSapphire = $("#table_sapphire");

    // With Placeholder
    $(".select2").select2({
        placeholder: "---- Pilih Salah Satu ----",
        theme: "bootstrap-5",
        dropdownParent: $("#modal-sapphire"),
        allowClear: true
    });

    // Add Button
    $(document).on("click", ".add-btn", function () {
        $(".form-sapphire").removeClass("was-validated");
        $("#modal-sapphire").modal("show");
        $(".modal-title").text("Form Tambah sapphire");
        $('.save-btn').show();
            ...
            $.ajax({
                ...
                success: function (res, status, xhr) {
                    ...
                    if (xhr.status == 200 && res.status) {
                        ...
                        if (typeof $tableSapphire !== 'undefined') {
                            $tableSapphire.bootstrapTable('refresh');
                        }
    ...
    $(document).on('click', '.save-btn', function (event) {
        ...
        let form = document.querySelector('.form-sapphire');
        let id = $('input[name="id"]').val();
        let url, type;

        if (id) {
            url = "{{ route('pintu.sapphire.update', ':id') }}".replace(':id', id);
            type = "PUT";
        } else {
            url = "{{ route('pintu.sapphire.create') }}";
            type = "POST";
        }
    ...
    // init table
    function initTable() {
        $tableSapphire.bootstrapTable("destroy").bootstrapTable({
            height: 500,
            locale: "en-US",
            search: true,
            showColumns: true,
    ...
    window.operateEvents = {
        'click .btn-edit': function (e, value, row, index) {
            $('#modal-sapphire').modal('show');
            $('.modal-title').text('Form Edit sapphire');
            $('.save-btn').html('<span class="fa fa-check"></span> Simpan').removeAttr('disabled');
        ...
        "click .btn-delete": function (e, value, row, index) {
            let url = "{{ route('pintu.sapphire.delete', ':id') }}";
