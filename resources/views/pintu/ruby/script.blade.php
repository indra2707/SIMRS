<script type="text/javascript">
    // Variable Name
    var $tableRuby = $("#table_ruby");

    // With Placeholder
    $(".select2").select2({
        placeholder: "---- Pilih Salah Satu ----",
        theme: "bootstrap-5",
        dropdownParent: $("#modal-ruby"),
        allowClear: true
    });

    // Add Button
    $(document).on("click", ".add-btn", function () {
        $(".form-ruby").removeClass("was-validated");
        $("#modal-ruby").modal("show");
        $(".modal-title").text("Form Tambah ruby");
        $('.save-btn').show();
            ...
            $.ajax({
                ...
                success: function (res, status, xhr) {
                    ...
                    if (xhr.status == 200 && res.status) {
                        ...
                        if (typeof $tableRuby !== 'undefined') {
                            $tableRuby.bootstrapTable('refresh');
                        }
    ...
    $(document).on('click', '.save-btn', function (event) {
        ...
        let form = document.querySelector('.form-ruby');
        let id = $('input[name="id"]').val();
        let url, type;

        if (id) {
            url = "{{ route('pintu.ruby.update', ':id') }}".replace(':id', id);
            type = "PUT";
        } else {
            url = "{{ route('pintu.ruby.create') }}";
            type = "POST";
        }
    ...
    // init table
    function initTable() {
        $tableRuby.bootstrapTable("destroy").bootstrapTable({
            height: 500,
            locale: "en-US",
            search: true,
            showColumns: true,
    ...
    window.operateEvents = {
        'click .btn-edit': function (e, value, row, index) {
            $('#modal-ruby').modal('show');
            $('.modal-title').text('Form Edit ruby');
            $('.save-btn').html('<span class="fa fa-check"></span> Simpan').removeAttr('disabled');
        ...
        "click .btn-delete": function (e, value, row, index) {
            let url = "{{ route('pintu.ruby.delete', ':id') }}";
